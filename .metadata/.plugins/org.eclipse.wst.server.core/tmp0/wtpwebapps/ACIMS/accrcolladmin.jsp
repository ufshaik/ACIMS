...
try
 {
  ...
  String s="select  Coll_Name,coll_code,Status,AFFILIATED from colleges where status!='requesting' and status!='onprocess'";
 // out.println(s);
  rs=stmt1.executeQuery(s);


  stmt2=con.createStatement();
  String s1="select count(*) from colleges where status='requesting'";
 // out.println(s1);
...
<TR> <TD vAlign=center align=middle colSpan=2>
   ...
   <TR>
                            ...
                            <FONT 
                              face="Verdana, Arial, Helvetica, sans-serif" 
                              color=#ffffff size=2>
                             <a style="color: black" class=links href="newrequests.jsp">New Requests(<%=rs1.getString(1)%>)</FONT></TD></TR>
...
<TD bgcolor="EFEFEF" align=left width="50%"><FONT face="Verdana, Arial, Helvetica, sans-serif" color=#000000 size=2>
     ...
     <%=rs.getString(3)%></font> 
	 <TD bgcolor="EFEFEF" align=center><FONT face="Verdana, Arial, Helvetica, sans-serif" color=#000000 size=2>
     <A href="reportinfoadmin.jsp?cc=<%=code%>">Report </font>
