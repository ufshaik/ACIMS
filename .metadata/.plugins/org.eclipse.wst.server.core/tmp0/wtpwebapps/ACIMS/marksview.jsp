...
while(ry.next())
	...
	if(i!=1)
	{
		 response.sendRedirect("viewmarks.jsp?&&v=1");
	}
...
while(rz.next())
	...
	if(j!=1)
	{
		 response.sendRedirect("viewmarks.jsp?&&v=2");
	}
pst=con.prepareStatement("select reg_id,firstname,lastname,marks from "+ta+" where course=? AND branch=?");
pst.setString(1,request.getParameter("year"));
pst.setString(2,request.getParameter("branch"));
rs=pst.executeQuery();
%>
...
<tr><td valign=left width="200" class=ma colspan=2 height=400><font color="white">&nbsp;&nbsp;&nbsp;&nbsp;<h5></h5>
...
<table  width="400" align=center border=0 bordercolor=white>
<caption><h3><font color="#336699">Attendance List</font></h3></caption>
<tr><td bgcolor="A9C6E2" height=20 align=center>REG_ID</td><td bgcolor="A9C6E2" height=20 align=center>NAME</td><td bgcolor="A9C6E2" height=20 align=center>Marks</td></tr>
<%while(rs.next()){%>

<tr><td bgcolor="#EFEFEF" height=20 align=center><%=rs.getString(1)%></td><td bgcolor="#EFEFEF" height=20 align=center><%=rs.getString(3)%>&nbsp;<%=rs.getString(2)%></td><td bgcolor="#EFEFEF" height=20 align=center><%=rs.getString(4)%></td></tr><%}%>
